...
create table if not exists public.orders (
  ...
  locale text not null check (locale in ('zh', 'en', 'es', 'fr', 'de')),
...
-- Localized paid-order email content is database-owned so delivery retries use
-- the exact same approved template as the original notification.
begin;
...
-- Payment/refund writers acquire business locks before this one shared lock;
-- reconciliation only takes the source-health lock.
begin;
