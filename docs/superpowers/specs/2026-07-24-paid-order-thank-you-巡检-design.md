...
网站只保留一个与此流程有关的最小字段：`orders.locale`，用于保存客户下单时选择的网站语言。登录客户和访客订单都必须保存这个值。
...
原有复杂 migration 026 撤回后，以一个新的最小 migration 026 仅保留 `orders.locale` 字段、五种语言约束和必要的历史数据回填，不保留任何数据库邮件发送逻辑。
