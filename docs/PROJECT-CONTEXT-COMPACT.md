...
## 16. BoxSofa 数据中心第一阶段（2026-07-23 至 2026-07-24）

- 新增商家专用数据中心 `/data-center`，只允许 `owner` 角色访问；服务端在输出私有 HTML 前完成权限判断，匿名访问只返回不跟随的登录跳转，页面 `no-store` 且禁止搜索引擎收录。
- 数据中心支持今日、7 天、30 天区间，展示真实 GMV、净销售额、订单数、访客数、转化率、待处理售后、告警和数据源新鲜度；顶部提供订单、客户、商品和售后工单的统一搜索。
- 统一财务口径：GMV 为 Stripe 已支付订单在退款前的商品总额；净销售额为 GMV 减去状态为 `succeeded` 的 Stripe 退款；所有金额在边界处按整数分计算。
- 网站访问与归因以服务端 `analytics_consents`、`analytics_events` 为唯一事实来源。没有分析同意或事件覆盖时，访客为 0、转化率显示不可用，不用模拟数据补齐。
- 已建立售后基础：退货、退款、换货、破损、配送、质量等工单，带状态机、负责人、到期时间、内部备注、乐观锁和管理员审计。后台填写退款金额只做记录，不会调用 Stripe。
- 已提供 PWA 清单和仅限 `/data-center` 的 service worker，应用名 `BoxSofa Data Center`，standalone 启动路径 `/data-center`，标准和 maskable 图标均为 512 × 512。原生安装仍需在支持安装提示的 Chrome/Edge 中从生产地址执行。
- 第一阶段暂不伪造社交平台观看量、点击量或发布状态；社交分析/发布、采购和扩展数据魔方保留为后续真实连接项。
- 生产数据库迁移已应用到 `025`。发布验证时发现并修复售后更新函数的 PL/pgSQL 同名列歧义；真实创建、更新和受控清理验证通过，残留测试数据为 0。远程发布门禁现逐条校验 25/25 个 Data Center 迁移版本、名称、生产指纹和历史修正源文件证明。
- 本地发布门禁通过：188/188 测试、迁移清单、PGlite、类型检查、正式构建、smoke 和 API 匿名权限审计全部通过；匿名数据中心必须为不跟随且精确指向 `/login` 的 307，受保护 API 只接受 401/403，不能用 503 掩盖运行故障。
- 完整审计：`docs/audits/2026-07-23-data-center-phase-1/README.md`；已审计实现提交：`459f79b`。
...
## 22. 数据中心站内导航修复（2026-07-24）

- 用户反馈数据中心内可点击功能会跳到旧 `/admin` 商家后台，离开后也没有清晰返回入口。根因是数据中心侧栏、手机底栏、售后客服入口和全局搜索结果都直接生成了 `/admin/...` 地址。
- 订单、产品、库存、客户、销售线索、客服消息、评价、邮件通知和操作日志现已复用真实商家功能并嵌入 `/data-center` 外壳，不再跳出数据中心；售后工单的“客服消息”也改为站内模块切换。
...
## 23. 社交自动发布（2026-07-27 11:00 Europe/Madrid）

- 本轮选择此前未记录为已发布或已预约的原始竖版 `D:\压缩沙发\沙发视频\可发布的视频\初始场景_-_2026-07-20_202607201824.mp4`（粉色压缩单椅拆箱；720 × 1280、12.130 秒、H.264/AAC 双声道）。快速检查确认文件可访问；画面从真空包装拆开到单椅成型，未见仿平台界面或第三方账号水印。原片未裁切、补边、重编码或加入画中画。
- 原片公开源在分支 `social-media/pink-armchair-20260727`：提交 `321926d`；Pinterest 因平台硬性视频封面要求，仅使用从原片提取的一帧，提交 `88ca998`。AiToEarn 视频媒体 `6a671ef73c1649a589f3a9db`，封面媒体 `6a671f0e3c1649a589f3aa6d`。
- YouTube Shorts 已发布：`https://www.youtube.com/watch?v=udcjK1-WbXk`（任务 `6a671f603c1649a589f3acb4`，flow `f051002a-f9d7-4209-bf09-8c3ab9a67ffd`，2026-07-27 11:07 Europe/Madrid）。TikTok 任务 `6a671f923c1649a589f3aeb7`（flow `f1750849-bc81-4fe9-ae4e-077a672609f6`）、Instagram Reels 任务 `6a671f9e3c1649a589f3af2f`（flow `6c209c37-a2b6-432c-b93d-efc7a4636bbc`）及 Pinterest video Pin 任务 `6a671fc13c1649a589f3b030`（flow `9182216a-5cfc-46fb-8cdf-16de97d145f1`）均已提交平台处理（status 2），尚未返回公开链接；下次仅查询并补录，禁止重建或重复上传。Pinterest 未附外链，以避开既有 spam 风险。
- 各平台文案使用对应 `pink_armchair_unboxing_20260727` UTM（Pinterest 无外链），仅陈述视觉产品演示、小户型/出租房、窄楼梯/小电梯、欧洲全境免费基础配送、Stripe 安全支付与预计 23–30 个工作日；无折扣、快速到货、真实客户案例或政策例外承诺。Facebook 因账号受限跳过：未登录、未重新授权、未发布、未补发、未创建替代账号。
