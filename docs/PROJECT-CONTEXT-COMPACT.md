...
- 新增商家专用数据中心 `/data-center`，只允许 `owner` 角色访问；服务端在输出私有 HTML 前完成权限判断，匿名访问返回安全的 404/401，页面 `no-store` 且禁止搜索引擎收录。
...
- 生产数据库迁移已应用到 `024`。发布验证时发现并修复售后更新函数的 PL/pgSQL 同名列歧义；真实创建、更新和受控清理验证通过，残留测试数据为 0。远程发布门禁现逐条校验 24/24 个 Data Center 迁移版本、名称和生产指纹。
- 本地发布门禁通过：186/186 测试、迁移清单、PGlite、类型检查、正式构建、smoke 和 API 匿名权限审计全部通过；匿名数据中心必须为不跟随跳转的 404，受保护 API 只接受 401/403，不能用 503 掩盖运行故障。
